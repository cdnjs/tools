...
func updateNpm(ctx context.Context, pckg *packages.Package) ([]newVersionToCommit, []version, error) {
	...
	log.Printf("existing npm versions: %s\n", strings.Join(existingVersionSet, ","))
	...
	if lastExistingVersion != nil {
		log.Printf("last existing version: %s\n", lastExistingVersion.Version)
		...
		if len(existingVersionSet) > 0 {
			...
			log.Printf("all existing versions not on npm: %s\n", *pckg.Name)
		}
...
func doUpdateNpm(ctx context.Context, pckg *packages.Package, versions []npm.Version) []newVersionToCommit {
	...
	for _, version := range versions {
		log.Printf("new version detected: %s\n", version.Version)
		tarball := npm.DownloadTar(ctx, version.Tarball)
		if err := storeGCS(path.Base(version.Tarball), tarball, pckg, version); err != nil {
			panic(err)
		}
	}
