...
func Invoke(ctx context.Context, e GCSEvent) error {
	...
	filemap := e.Metadata["autoupdate-filemap"].(string)

	url := fmt.Sprintf("https://storage.googleapis.com/%s/%s", e.Bucket, e.Name)

	if err := publish(url, pkg, version, filemap); err != nil {
...
type Message struct {
	...
	// Files that will be copied on the CDN
	Filemap []FileMap `json:"filemap"`
}

func publish(tar, pkg, version, filemap string) error {
	...
	signedURL, err := generateV4SignedURL(ctx, pkg, version,
		fmt.Sprintf("%s/%s/files.tgz", pkg, version))
	if err != nil {
		return errors.Wrap(err, "could not generate signed URL")
	}

	var filemapStruct []FileMap
	if err := json.Unmarshal([]byte(filemap), &filemapStruct); err != nil {
...
func generateV4SignedURL(ctx context.Context, pkg string, version string, dst string) (string, error) {
