...
func generateV4SignedURL(ctx context.Context, dst string) (string, error) {
	...
	log.Fprintln("Generated PUT signed URL:")
	log.Fprintf("%q\n", url)
	log.Fprintln("You can use this URL with any user agent, for example:")
	log.Fprintf("curl -X PUT -H 'Content-Type: application/octet-stream' --upload-file my-file %q\n", url)
