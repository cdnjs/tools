...
func publish(tar, pkg, version string) error {
	...
	signedURL, err := generateV4SignedURL(fmt.Sprintf("%s/%s/", pkg, version))
	if err != nil {
...
func generateV4SignedURL(dst string) (string, error) {
	...
	opts := &storage.PostPolicyV4Options{
		...
		Expires:        time.Now().Add(10 * 24 * time.Hour), // 10 days
	...
	pv4, err := storage.GenerateSignedPostPolicyV4(OUTGOING_BUCKET, dst, opts)
	if err != nil {
