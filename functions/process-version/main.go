...
func publish(tar, pkg, version string) error {
	...
	signedURL, err := generateV4SignedURL(ctx, fmt.Sprintf("%s/%s/files.tgz", pkg, version))
	if err != nil {
...
func generateV4SignedURL(ctx context.Context, dst string) (string, error) {
	...
	log.Println("Generated PUT signed URL:")
	log.Printf("%q\n", url)
	log.Println("You can use this URL with any user agent, for example:")
	log.Printf("curl -X PUT -H 'Content-Type: application/octet-stream' --upload-file my-file %q\n", url)
