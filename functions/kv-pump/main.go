...
func updatePackage(ctx context.Context, cfapi *cloudflare.API, pkgName string,
	version string, newFiles []string) error {
	...
	if err != nil && strings.Contains(err.Error(), "key not found") {
		...
		// package doesn't have any versions in cdnjs, assume the current one is the latest
		pkg.Version = &version
		pkg.Assets = []packages.Asset{
			{
				Version: version,
				Files:   newFiles,
			},
