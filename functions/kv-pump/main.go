...
func Invoke(ctx context.Context, e GCSEvent) error {
	...
	if err := updatePackage(ctx, cfapi, pkgName); err != nil {
		log.Fatalf("failed to update aggregated metadata: %s", err)
	}
...
func updatePackage(ctx context.Context, cfapi *cloudflare.API, pkgName string) error {
	pkg, err := kv.GetPackage(ctx, cfapi, pkgName)
	if err != nil {
		return errors.Wrap(err, "could not retrieve package")
	}
	if pkg == nil {
		// FIXME: get package from config
		return errors.New("package doesn't exist yet")
	}
	...
	if err := updateFilenameIfMissing(ctx, cfapi, pkg); err != nil {
...
func updateFilenameIfMissing(ctx context.Context, cfapi *cloudflare.API, pkg *packages.Package) error {
	key := pkg.LatestVersionKVKey()
	assets, err := kv.GetVersion(ctx, cfapi, key)
	if err != nil {
		// All package metadata will be in KV, so this error should never occur.
		return (errors.Wrapf(err, "KV metadata not found for latest version `%s`", key))
	}

	if len(assets) == 0 {
		return errors.Errorf("KV version `%s` contains no assets", key)
	}
