...
func Invoke(ctx context.Context, e gcp.GCSEvent) error {
	...
	if err := audit.WroteKV(ctx, pkgName, version, sris, newKVFiles, string(configStr)); err != nil {
...
func updateAggregatedMetadata(ctx context.Context, cfapi *cloudflare.API,
	...
	kvWrites, _, err := kv.UpdateAggregatedMetadata(cfapi, ctx, *pkg.Name, version, newAssets)
	if err != nil {
