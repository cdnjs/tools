...
func GitClone(ctx context.Context, pckg *Package, gitpath string, partialClone bool) ([]byte, error) {
	args := []string{"clone"}
	if partialClone {
		args = append(args, []string{"--filter=blob:none", "--no-checkout"}...)
	}
	args = append(args, []string{*pckg.Autoupdate.Target, "."}...)
