...
func uploadToOutgoing(content bytes.Buffer, msg Message) error {
	configBytes, err := json.Marshal(msg.Config)
	if err != nil {
		return errors.Wrap(err, "could not marshall config")
	}
	...
	log.Printf("config is %s", string(configBytes))
	req.Header.Set("x-goog-meta-package", msg.Pkg)
	req.Header.Set("x-goog-meta-version", msg.Version)
	req.Header.Set("x-goog-meta-config", string(configBytes))
...
func writeConfig(dstDir string, config packages.Package) error {
	configBytes, err := json.Marshal(config)
	if err != nil {
		return errors.Wrap(err, "could not marshall config")
	}
	if err := ioutil.WriteFile(path.Join(dstDir, "config.json"), configBytes, 0644); err != nil {
