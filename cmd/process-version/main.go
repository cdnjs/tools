...
func main() {
	...
	if config.Autoupdate != nil && config.Autoupdate.FileMap != nil {
		filemap = config.Autoupdate.FileMap
	}

	files, err := getInfiles(ctx, filemap)
	if err != nil {
...
func getInfiles(ctx context.Context, filemap []packages.FileMap) ([]string, error) {
	files := make([]string, 0)
	...
	for _, fileMap := range filemap {
		for _, pattern := range fileMap.Files {
			...
			for _, f := range list {
				...
				files = append(files, path.Join(*fileMap.BasePath, f))
			}
		}
	}
